/**
 * Browser-side rendering logic (ESM port of src/renderer.js).
 * No Node.js dependencies — uses marked from npm.
 */
...
function parseRatio(ratioText) {
    const m = /^(\d+(?:\.\d+)?)\s*:\s*(\d+(?:\.\d+)?)$/.exec(String(ratioText || "").trim());
    if (!m) throw new Error(`Invalid ratio: ${ratioText}`);
    const w = Number(m[1]), h = Number(m[2]);
    if (!Number.isFinite(w) || !Number.isFinite(h) || w <= 0 || h <= 0) throw new Error(`Invalid ratio: ${ratioText}`);
    return { ratioW: w, ratioH: h };
}

function resolveDimensions({ ratioW, ratioH }) {
...
function textScaleByLength(length) {
    ...
    return clamp(1 + ((0.78 - 1) / (620 - 120)) * (length - 120), 0.78, 1);
}
...
function extractHeroContent(card) {
    ...
    const pushByMarker = (text, { preferPoint = false } = {}) => {
        ...
        if (!raw) return;
        const m = raw.match(/^\[(logo|kicker|center|left|right|bottom|point)\]\s*(.+)$/i);
        if (m) {
            const key = m[1].toLowerCase();
            const value = parseInlineText(m[2]);
            if (!value) return;
...
export function renderMarkdownToCards(markdown, opts = {}) {
    ...
    const htmlCards = cards.map((card) => {
        ...
        const cardTheme = cardBgImage ? { ...theme, backgroundImage: cardBgImage } : theme;
